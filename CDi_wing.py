...
def CL_w_eta_t(W,V,rho,S):
    CL_actual=2*W/rho/V/V/S
    return [CL_actual]
...
def R_LE_suct_param(mach,rho,vinf,rle,mu,AR,Span,Sweep,ctip,croot):
    ...
    Rsucrat=np.interp(ARLambda, ARLam, R, left=np.nan, right=np.nan)
...
def v_w_factor(AR,mach,Sweep,taper):
    ...
    vmid=np.array([0, 0, 0, 0, 0, 0, 0])
    vmid = np.array([np.interp(sweeptan, sweepparam, vbase[i, :], left=np.nan, right=np.nan) for i in range(vbase.shape[0])]) # chatgpt translation

        
    # do final interpolation for v (induced drag factor due to linear twist)
    v = np.array([np.interp(taper, _lambda, vmid[i, :], left=np.nan, right=np.nan) for i in range(vmid.shape[0])])
    ...
    bwmid=np.array([0, 0, 0, 0, 0, 0, 0])
    bwmid = np.array([np.interp(sweeptan, sweepparam, bwbase[i, :], left=np.nan, right=np.nan) for i in range(bwbase.shape[0])]) # chatgpt translation

    # do final interpolation for w (zero lift drag factor due to linear twist)

    bw = np.array([np.interp(taper, _lambda, bwmid[i, :], left=np.nan, right=np.nan) for i in range(bwmid.shape[0])]) # chatgpt translation
    w=bw/beta
    return v,w
...
def CDiw_calculate(CL_w,AR,eta_t,spaneff,v,w):
