...
def calcRwf(mach, re):
    ...
    rwfm = np.array([0, 0, 0, 0, 0, 0, 0])
    #
    # mach 0.25 curve fit
    rwfm[1]=(0.98738954-0.22335005*np.log(re)+0.0189956*(np.log(re))**2-0.00071991301*(np.log(re))**3+1.0258411e-5*(np.log(re))**4)/(1.0-0.22716486*np.log(re)+0.019401223*(np.log(re))**2-0.0007383189*(np.log(re))**3+1.0563041e-5*(np.log(re))**4)
    #
    # mach 0.40 curve fit
    rwfm[2]=(1.0499334-0.12161303*np.log(re)+0.0035277416*(np.log(re))**2)/(1.0-0.112725*np.log(re)+0.0029917867*(np.log(re))**2+1.091304e-5*(np.log(re))**3)
    #
    # mach 0.60 curve fit
    rwfm[3]=(0.86828712685455-0.18845483203009*np.log(re)+0.0153254133318254*(np.log(re))**2-0.00055346095362923*(np.log(re))**3+7.49008313386838e-6*(np.log(re))**4)/(1.0-0.22438644552757*np.log(re)+0.0192423512899198*(np.log(re))**2-0.000766748565364264*(np.log(re))**3+1.329171731769246e-5*(np.log(re))**4-6.3067958874473e-8*(np.log(re))**5)
    #
    # mach 0.70 curve fit
    rwfm[4]=(0.90322124-0.10780675*np.log(re)+0.0032322543*(np.log(re))**2)/(1.0-0.12386001*np.log(re)+0.0041251962*(np.log(re))**2-1.668079e-5*(np.log(re))**3)
    #
    # mach 0.80 curve fit
    rwfm[5]=(0.81875278-0.094299175*np.log(re)+0.0027309697*(np.log(re))**2)/(1.0-0.12335048*np.log(re)+0.0042868258*(np.log(re))**2-2.7863927e-5*(np.log(re))**3)
    #
    # mach 0.85 curve fit
    rwfm[6]=(0.94758681-0.21700704*np.log(re)+0.018794158*(np.log(re))**2-0.00072928383*(np.log(re))**3+1.0693582e-5*(np.log(re))**4)/(1.0-0.22764419*np.log(re)+0.01959818*(np.log(re))**2-0.00075606298*(np.log(re))**3+1.1024387e-5*(np.log(re))**4)
    #
    # mach 0.90 curve fit
    rwfm[7]=(0.94027346-0.21838712*np.log(re)+0.019174732*(np.log(re))**2-0.00075384048*(np.log(re))**3+1.1189366e-5*(np.log(re))**4)/(1.0-0.2304395*np.log(re)+0.020080209*(np.log(re))**2-0.00078380017*(np.log(re))**3+1.1556739e-5*(np.log(re))**4)
    #
