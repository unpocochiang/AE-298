...
def CDo_htail(re,mach, sweep,tc_max_loc_h, tc_avg, sref, swet, weight, vinf, ctip,croot,Span, Wsref, rho, tcmax):
    cf = calcCf(re,mach)
    rls = calcRls(mach,sweep)
    CDo_htail_val = CalcCDow(cf,rls,tc_max_loc_h,tc_avg,sref,swet)
    CDw_htail_val = CalcCDwave(mach,weight,vinf,rho,sweep,tcmax,ctip,croot,
                               Wsref,Span)
    
    CDo_trans = CDw_htail_val + CDo_htail_val
    CDo_htail_val = fcn.fcn(mach, CDo_htail_val, CDo_trans)
