...
tc_average = 0.06 #set value by Dr.Elle. Im not sure where it came from
sweep = 50
tcmax = 0.15
...
def trans_vtail_Cdo_cdw(re, tc_avg, sref, swet, mach, weight, vinf, rho, sweep, tcmax, ctip, croot, span):
    cf = CDo_vtail_trans.trans_calcCf(re, mach)
    maxtcloc = 0.4
    CDo_vtail_trans_val = CDo_vtail_trans.CalcCDow_transonic(cf,maxtcloc,tc_avg,sref,swet)
    CDw_vtail = CDo_vtail_trans.CalcCDwave(mach,weight,vinf,rho,sweep,tcmax,ctip,croot,sref,span)
    return CDo_vtail_trans_val + CDw_vtail

def CDo_ply(num_ply, pylon_arrangement,t_nac, dn,l_py,re, mach, sref, tc_avg, htail_sweep, vtail_sweep,weight,vinf, rho,ctip, croot, span):
    tc_avg = 0.06 #set value by Dr.Elle. Im not sure where it came from
    htail_sweep = 50
    vtail_sweep = 50
    tcmax = 0.15
    horiz, vert = check_pylon_arrangement(pylon_arrangement)
    swet= Est_pylon_wet_area(t_nac, dn,l_py)
    sub_htail_Cdo_val = sub_htail_Cdo(re, mach, sref, swet, tc_avg, htail_sweep)
    trans_htail_Cdo_Cdw_val = trans_htail_Cdo_Cdw(re, tc_avg, sref, swet, mach, weight, vinf, rho, htail_sweep, tcmax, ctip, croot, span)
    sub_vtail_Cdo_val = sub_vtail_Cdo(re, mach, sref, swet, tc_avg, vtail_sweep)
    trans_vtail_Cdo_cdw_val = trans_vtail_Cdo_cdw(re, tc_avg, sref, swet, mach, weight, vinf, rho, vtail_sweep, tcmax, ctip, croot, span)
