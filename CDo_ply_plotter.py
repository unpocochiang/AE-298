...
for i, alt in enumerate(altitude):
    ...
    for k, m in enumerate(mach):
        print(k)
        print(f'm:{m}')
        re = re_calc.re(density, m, erj_data.l_pyl, visc, temp) # float for each mach
...
plt.title('CDo of Pylon') # gonna do some LaTeX stuff with this in a bit, but this is a proof of concept lol
